...
2.创建实例，选择类型和配置
服务器选择Linux Centos 8.x 以上的，自带dnf
...
报错解决
```
Sep 12 03:47:04 iZwz982ksp86xf88ituqq8Z systemd[1]: jenkins.service: Failed with result 'protocol'.
Sep 12 03:47:04 iZwz982ksp86xf88ituqq8Z systemd[1]: Failed to start Jenkins Continuous Integration Server.

1.先卸载已经按照的Jenkins
dnf remove jenkins
2.安装指定版本的Jenkins（发现dnf module list 不支持指定Jenkins,寻找其他安装方法

//这是在网上随便找的版本(不能选版本太低的，会在安装过程中插件无法安装的情况)
wget https://prodjenkinsreleases.blob.core.windows.net/redhat/jenkins-2.346-1.1.noarch.rpm
...
#### 4.2.2 输入任务名称，选中**Freestyle project** 点击确定

#### 4.2.3 源码管理 选择Git  输入Git仓库
...
```
Repository URL:https://github.com/Peach-Lu/vue3-ts-cms
Credentials
```
...
4. Expiration 选择No Expiration 或者90天
5. **Select scopes** 选择  repo全选
6. 复制token 填入 password
...
点击Manage Jenkins 选中 Manage Plugins

点击Available plugins 搜索node

勾选node 点击Download now and install after restart

点击安装饿按成后重启 jenkins

重启完成后 点击系统管理 - 全局工具配置

Nodejs - 新增Nodejs 填写node14  选择node版本 我这里选择的是14.18.1  保存

回到首页 点击项目 点击配置

选择构建环境 勾选Provide Node & npm bin/ folder to PATH 选中node14
...
# 删除/root/mall_cms文件夹里所有的内容  需要root用户
rm -rf /root/mall_cms/* 
